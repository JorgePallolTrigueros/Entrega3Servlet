<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>Employee Directory</title>
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">

</head>
<body>
<jsp:include page="navbar.jsp"></jsp:include>
	<div class = "mt-5 mb-5 container">
		
		<h1>Direccion Detalles</h1> 
		<hr/>
		
		<p>${NOTIFICATION}</p>
		<p>Usuario: ${user.first_name}</p>
		<p>Direccion: ${direccion.direccion_personal}</p>
		<p>Direccion 2: ${direccion.direccion_personal2}</p>
	
		<!--  Hay que hacer la tabla para one to many -->
		<!--  ${direcciones} igualita a la lista de user si fuera una relacion one to many pero debolvemos un obj al ser one to one-->
	 <a href="/entregam3/UserController">Volver Inicio</a></br>	
	 
	 
	 						<a href = "${pageContext.request.contextPath}/direccion?action=u&id=${direccion.id}">Edit</a> 
						| 
						<a href = "${pageContext.request.contextPath}/direccion?action=d&id=${direccion.id}">Delete</a> 
						
						<button class = "btn btn-primary" onclick="window.location.href = 'views/direccion-create.jsp'">Add TODO</button>
						
						

	 
	 
	 	
				
	</div>
<jsp:include page="footer.jsp"></jsp:include>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"></script>
</body>
</html>